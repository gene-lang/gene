...
llama_sampler *build_sampler(float temperature, float top_p, int top_k, uint32_t seed) {
...
int32_t tokenize_prompt(const llama_vocab *vocab,
...
void fill_completion(const std::string &text,
    ...
    if (!tokens.empty()) {
        out_completion->tokens = static_cast<char **>(std::malloc(tokens.size() * sizeof(char *)));
...
gene_llm_status gene_llm_new_session(gene_llm_model *model,
    ...
    const int ctx_len = options && options->context_length > 0 ? options->context_length : model->default_ctx;
    ctx_params.n_ctx = ctx_len;
    ctx_params.n_batch = options && options->batch_size > 0 ? options->batch_size : std::min(512, ctx_len);
    ...
    session->default_max_tokens = options && options->max_tokens > 0 ? options->max_tokens : 256;
    ...
    session->default_seed = options && options->seed != 0 ? static_cast<uint32_t>(options->seed) : LLAMA_DEFAULT_SEED;
...
gene_llm_status gene_llm_infer(gene_llm_session *session,
    ...
    const int max_tokens = options->max_tokens > 0 ? options->max_tokens : session->default_max_tokens;
    if (max_tokens <= 0) {
        fill_completion("", {}, GENE_LLM_FINISH_CANCELLED, 0, out_completion);
        return GENE_LLM_OK;
    }

    const float temperature = options->temperature > 0.0f ? options->temperature : session->default_temperature;
    const float top_p = options->top_p > 0.0f ? options->top_p : session->default_top_p;
    const int top_k = options->top_k > 0 ? options->top_k : session->default_top_k;
    const uint32_t seed = options->seed > 0 ? static_cast<uint32_t>(options->seed) : session->default_seed;
    ...
    if (llama_model_has_encoder(session->model->model)) {
        ...
        llama_token decoder_start = llama_model_decoder_start_token(session->model->model);
    ...
    for (int generated = 0; generated < max_tokens; ++generated) {
        ...
        char buffer[384];
        const int piece_len = llama_token_to_piece(vocab, new_token, buffer, sizeof(buffer), 0, true);
        if (piece_len < 0) {
            llama_sampler_free(sampler);
    ...
    fill_completion(completion_text, token_texts, finish_reason, latency_ms, out_completion);
