...
This protocol enables multiple Claude agent instances to collaborate on a single codebase by communicating through shared markdown files. Each agent operates independently in its own session but maintains coordination through a shared conversation log.
...
1. **Request your identity from the user**:
   ...
   - Am I joining an existing collaboration?
   
   If this is an existing collaboration, I'll read the conversation history to catch up, otherwise I'll start a new collaboration.
   ```
...
4. **Remember your name**: Store your assigned name in memory and use it consistently throughout the entire session.

## File Structure

### conversation.md
- **Location**: `.multi_agent_conversation.md`
...
### convhistory.md
- **Location**: `.multi_agent_history.md`
...
4. **Assess staleness**:
   - Messages < 5 minutes old: Very fresh, proceed confidently
   - Messages 5-30 minutes old: Recent, likely still relevant
   - Messages 30+ minutes old: Check with user before proceeding
   - Messages hours/days old: **MUST confirm with user** before acting

5. **If timestamps are stale, ask user**:
   ```
   I see the last message in conversation.md is from [TIMESTAMP], which was [X hours/minutes] ago.
...
All messages in conversation.md must follow this format:
...
When writing to conversation.md:
...
5. **Reference specific files, functions, or line numbers when relevant**
...
- conversation.md exceeds ~500 lines
...
1. **Check if archiving is needed**:
   - Count lines in conversation.md
   - If > 500 lines, proceed to step 2

2. **Request user permission**:
   ```
   conversation.md has grown to ~XXX lines. Should I archive older messages 
   to convhistory.md to keep context focused? 
...
3. **If approved, perform archiving**:
   ...
   a. Read current conversation.md
   ...
   c. **Critical: Keep these in conversation.md**:
   ...
   d. Append archived content to convhistory.md:
      ```markdown
   ...
   e. Remove archived messages from conversation.md
   
   f. Add archive marker to conversation.md:
      ```markdown
      [TIMESTAMP] @YourName: Archived messages before [CUTOFF_TIME] to convhistory.md
      ```

4. **Verify archiving**:
   - Ensure conversation.md still has context for current work
   - Confirm convhistory.md has complete archived content
   - Report completion to @user

### Referencing Archived Content

When you need historical context:
1. Read convhistory.md
2. Search for relevant timestamps or agent names
3. Reference in your messages:
   ```markdown
   [2025-11-10 15:00] @Builder → @user: Per discussion in convhistory.md [2025-11-09 14:30], 
   I've implemented the caching strategy we agreed on.
   ```
...
4. **After Completing Work**
   ...
   - Update conversation.md with results and completion summary
   - Commit all changes (including conversation.md updates)
...
2. **Check for concurrent work**:
   - Read conversation.md before editing shared files
...
- **Read before acting**: Always check conversation.md before starting work
...
### Shared Context
- conversation.md is the ONLY shared memory between sessions
- All coordination happens through conversation.md
...
### Context Efficiency
- Keep conversation.md under 500 lines
- Archive older content to convhistory.md
...
If conversation.md doesn't exist:
...
If you find unclear messages in conversation.md:
...
2. Ask @user before proceeding:
   ```
   The most recent message in conversation.md is from [X hours/days] ago.
