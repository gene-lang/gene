...
These files are not the canonical description of current behavior.

Use `spec/` for implemented language behavior and the root [`docs/`](../README.md)
directory for current implementation/reference docs. The files here are kept for
proposal work, subsystem design context, or historical reference.
...
- `implemented/` contains docs for features that exist in the codebase, but
...
- `archive/` contains superseded or historical notes that remain useful for
  context, including removed features and scratchpad material.
