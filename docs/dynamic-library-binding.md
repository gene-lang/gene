...
- `^abi` defaults to `"cdecl"`; v1 rejects other ABI names.
...
| `Bool` | zero/non-zero boolean argument or return |
| `String` | temporary NUL-terminated `const char*` argument, copied `cstring` return |
