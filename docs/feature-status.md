...
| Gradual type system | Beta | `docs/type-system-mvp.md`, `docs/how-types-work.md`, `spec/02-types.md` | Compile-time checking, runtime descriptor validation, typed arguments, locals, returns, typed properties, enum type annotations, and typed enum payload validation are partially delivered. | Type checker and typed tests exist in `tests/test_type_checker.nim`, `testsuite/02-types/`, and typed function tests. | Canonical descriptor pipeline, negative-path coverage, flow typing breadth, generic classes, and complete type inference remain incomplete. | Use explicit annotations at boundaries; expect beta-level churn. |
...
| Pattern matching | Beta | `spec/12-patterns.md` | The documented Beta subset covers direct argument binding, `var` array/Gene destructuring, simple `case/when`, and enum ADT `case` patterns. Enum cases support qualified and unambiguous bare variants, declaration-order payload binding, ignored `_` payload binders, unit variants, diagnostics for ambiguity/arity/unknown variants, and exhaustiveness diagnostics for statically known enum cases. The standalone `(match ...)` expression is removed. | Pattern coverage exists in `tests/integration/test_pattern_matching.nim`, `tests/integration/test_case.nim`, `tests/integration/test_enum.nim`, `tests/test_type_checker.nim`, and `testsuite/12-patterns/`. | Future pattern-language work includes nested patterns beyond covered destructuring, guards, map destructuring, broad function-parameter patterns, literal quote patterns, or-patterns, as-patterns, and any reintroduced standalone `match` expression; broad non-enum diagnostics remain maturing. | Use the documented Beta subset for durable code; use `(var pattern value)` for binding and `(case ...)` for branching; do not use `(match ...)`; treat advanced pattern forms as Future. |
