...
- [ ] 1.5 Make `$ex` thread-local in the runtime (coordinate with thread support).
- [ ] 1.6 Implement the `synchronized` form with optional `^on` property for global locking.
- [ ] 1.7 Add direct-child global lock management for `^on` targets, and treat omitted `^on` as a global lock.
...
- [ ] 2.5 Add concurrent tests to verify `synchronized` blocks access to the locked global child and allows other globals, including the omitted `^on` global lock behavior.
